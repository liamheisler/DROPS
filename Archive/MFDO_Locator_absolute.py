...
def findOffsets(rmat, tvec):
    ...
    #tvec = np.dot(tvec[0], rmat)
    #rmat = np.linalg.inv(rmat)
    tvec = np.dot(rmat, tvec[0].T)

    print(tvec)
    
    x = round(0.1 * tvec[0][0], 3) #cm
    y = round(0.1 * tvec[1][0], 3) #cm
    z = round(0.1 * tvec[2][0], 3) #cm
...
while True:
    ...
    cv2.putText(frame, "+", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
